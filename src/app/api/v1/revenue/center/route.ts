...
export const GET = withAuth(async (_request, { user }) => {
  try {
    ...
    console.error("[revenue/center]", err);
    const { error } = await import("@/lib/api/response");
    const message = err instanceof Error ? err.message : "Failed to load revenue center.";
    return error(message, "REVENUE_CENTER_ERROR", 500);
  }
}, "revenue.manage");
